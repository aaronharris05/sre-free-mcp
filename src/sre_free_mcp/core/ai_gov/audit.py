"""AI Governance audit — sweep orchestrator."""
    ...
    *,
    ...
    for wf in workflows:
        findings.extend(
            evaluate(
                wf,
                business_purpose_min_chars=business_purpose_min_chars,
                require_policy_refs=require_policy_refs,
            )
        )
    ...
    if findings and write:
        _write_findings_shared(
            ...
            generated_at=now,
            tables=tables,
            source="sre_free_mcp.core.ai_gov",
        )

    logger.info(
        "ai_gov_complete workflows=%d findings=%d",
        len(workflows),
        len(findings),
    )
    return AuditResult(
